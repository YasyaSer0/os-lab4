...
Для визначення процесу, який використовує найбільше оперативної пам’яті, можна використати команду:
```bash
ps aux --sort=-%mem | head
```
Ця команда:
- ps aux - виводить повний список процесів;
- --sort=-%mem - сортує процеси за спаданням використання пам’яті;
- head - показує перші рядки списку.

<img width="812" height="404" alt="image" src="https://github.com/user-attachments/assets/94cf759e-16fd-4a44-b02f-d47dce575791" />

Пояснення:

У списку першим знаходиться процес, який використовує найбільший обсяг пам’яті.

У моєму випадку це процес:
```bash
xfwm4
```
У колонці %MEM видно, що він використовує приблизно 5.5 % оперативної пам’яті.

---

Динамічний моніторинг у реальному часі

Для моніторингу в реальному часі використовується команда:
```bash
top
```
Вона постійно оновлює інформацію про:
- завантаження CPU
- використання пам’яті
- активні процеси

<img width="823" height="534" alt="image" src="https://github.com/user-attachments/assets/341a0adb-d8ae-45e0-a6f4-20a6652cbf62" />

Пояснення:

У колонці %MEM відображається відсоток використання пам’яті кожним процесом.

На моєму скріні найбільше пам’яті використовує процес:
```bash
Xorg
```
Він споживає приблизно 5.3 % від загального обсягу оперативної пам’яті.

**3. Як отримати ієрархію батьківських процесів? Опишіть її структуру.**

Для перегляду ієрархії процесів було виконано команду:
```bash
ps -ejH | head -20
```
Де:
- ps - відображає процеси;
- -e - показує всі процеси системи;
- -j - додає розширену інформацію (PGID, SID);
- -H - відображає процеси у вигляді ієрархічної структури;
- head -20 - показує перші 20 рядків.

<img width="779" height="493" alt="image" src="https://github.com/user-attachments/assets/19f49a9f-7626-45b3-b27e-41ac902cc6f6" />
...
Ієрархія процесів у Linux має деревоподібну структуру:
- на вершині знаходиться процес systemd (PID 1);
- від нього створюються системні служби;
- кожен процес має батьківський процес (PPID);
- дочірні процеси створюються через механізм fork.

У моєму випадку на скріні видно системні процеси ядра, такі як:
- kthreadd
- kworker
- rcu_tasks_kthread
- migration
...
Це службові процеси, які забезпечують роботу ядра Linux.

Структура ієрархії виглядає так:
```bash
systemd (PID 1)
 ├── служби
 │    ├── дочірні процеси
 │    │     └── їхні дочірні процеси
```

Таким чином, всі процеси в системі пов’язані між собою у вигляді дерева, де кожен процес має свого батька.

**4. Чим відрізняється команда top від ps?**

Для аналізу процесів у Linux використовуються команди ps та top, але вони працюють по-різному.
...
<img width="373" height="133" alt="image" src="https://github.com/user-attachments/assets/11791e7a-10a3-4998-926a-0e5fbfad4da7" />

Пояснення:

Команда ps:
- показує список процесів лише на момент виконання;
- не оновлює інформацію автоматично;
- відображає статичний знімок стану процесів;
- за замовчуванням показує тільки процеси поточного терміналу.

На скріні видно лише кілька процесів:
- bash
- ps

Тобто команда показала стан системи в конкретний момент часу.
...
<img width="843" height="533" alt="image" src="https://github.com/user-attachments/assets/2507688d-5a3d-486e-8f79-05e3694853e3" />
...
- показує завантаження CPU;
- відображає використання оперативної пам’яті;
- дозволяє бачити найбільш активні процеси.

На скріні видно:
- кількість процесів (Tasks);
- завантаження процесора (%Cpu);
- використання пам’яті (MiB Mem);
- список процесів із колонками %CPU та %MEM.

Основна різниця

| ps                   | top                   |
| -------------------- | --------------------- |
| Статичний знімок     | Динамічний моніторинг |
| Оновлюється один раз | Оновлюється постійно  |
| Простий вивід        | Інтерактивний режим   |

**5. Які додаткові можливості має htop порівняно з top?**

htop - це покращена версія top, яка має більш зручний інтерфейс та розширені можливості керування процесами.
...
<img width="1129" height="761" alt="image" src="https://github.com/user-attachments/assets/9f434486-74d1-4ca4-a2d2-5f5eab512ea9" />

---

1. Кольоровий інтерфейс

У верхній частині екрана видно:
- кольорові шкали використання CPU;
- використання оперативної пам’яті (Mem);
- використання Swap;
- кількість процесів (Tasks);
- Load average та Uptime.

Це робить моніторинг більш наочним порівняно з top.

---

2. Відображення процесів у вигляді дерева (F5)

Для перегляду ієрархії процесів було натиснуто:
```bash
F5
```

<img width="1049" height="751" alt="image" src="https://github.com/user-attachments/assets/1ecce0e7-6606-41a4-8c1a-319e6abde9c9" />

Пояснення:

У цьому режимі видно:
- структуру процесів;
- батьківські та дочірні процеси;
- вкладеність служб.

У top такого зручного режиму немає.

---

3. Завершення процесу через меню (F9)

Було обрано процес та натиснуто:
```bash
F9
```

<img width="1131" height="748" alt="image" src="https://github.com/user-attachments/assets/22ce02e5-1d09-474c-a62f-233fc4efece4" />

Пояснення:

Відкривається меню сигналів:
- SIGTERM
- SIGKILL
- SIGSTOP
- SIGCONT та інші

Це дозволяє завершити процес без введення команди kill вручну.

---

4. Додаткові можливості htop
- навігація стрілками ↑ ↓
- пошук процесів (F3)
- фільтрація (F4)
- сортування (F6)
- зміна пріоритету (F7/F8)
- зручний вихід (F10)
...
| top                       | htop                      |
| ------------------------- | ------------------------- |
| Текстовий інтерфейс       | Кольоровий інтерфейс      |
| Обмежена навігація        | Повна навігація стрілками |
| kill через команду        | F9 меню сигналів          |
| Немає повноцінного дерева | F5 режим дерева           |

 ---
 
Висновок

htop є більш сучасною та функціональною утилітою для моніторингу процесів.

Він забезпечує зручне керування, наочність та розширені можливості у порівнянні з top.
...
- App Switcher (меню відкритих програм) - дозволяє переглядати активні та нещодавно використані додатки.
  ...
  <img width="588" height="1280" alt="image" src="https://github.com/user-attachments/assets/b487f2f9-05fc-46f2-b91e-fe9ac06bc02c" />

- Налаштування → Акумулятор - показує, які додатки використовують найбільше ресурсів та енергії.

  <img width="588" height="1280" alt="image" src="https://github.com/user-attachments/assets/5c5e5acb-e2a2-4630-a724-bae061b766e3" />

- Налаштування → Основні → Сховище iPhone - відображає використання пам’яті додатками.

  <img width="727" height="1280" alt="image" src="https://github.com/user-attachments/assets/3a0d80fb-e391-47a1-8bc9-87a3a81e05d9" />
