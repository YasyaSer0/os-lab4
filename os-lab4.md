...
Директорія /proc - це спеціальна віртуальна файловa система в Linux, яка містить інформацію про стан системи та запущені процеси.

Вона не зберігає файли у звичайному розумінні, а формує їх динамічно під час роботи системи.

У ній можна знайти:
- інформацію про процесор (/proc/cpuinfo);
- інформацію про оперативну пам’ять (/proc/meminfo);
- інформацію про кожен запущений процес (папки з номерами PID);
- параметри ядра системи.

Через /proc можна аналізувати стан системи та виконувати її моніторинг.
